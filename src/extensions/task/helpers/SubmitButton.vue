<template>
  <div :class="backgroundColor" class="flex flex-row container rounded-lg items-start p-2">
    <Transition appear>
      ...
      >
        <svg :class="fontColor" class="w-8 h-8 my-0 mx-2" fill="none" stroke="currentColor"
             stroke-width="1.5"
             viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path v-if="['correct'].includes(state.state)" d="M4.5 12.75l6 6 9-13.5"
                stroke-linecap="round"
                stroke-linejoin="round"/>
          <path v-if="['incorrect'].includes(state.state)"
                d="M15 15l6-6m0 0l-6-6m6 6H9a6 6 0 000 12h3"
                stroke-linecap="round"
                stroke-linejoin="round"/>
          <path v-if="['final-incorrect'].includes(state.state)" d="M6 18L18 6M6 6l12 12"
                stroke-linecap="round"
                stroke-linejoin="round"/>
        </svg>
      </div>
    ...
    <div :class="{ 'basis-3/5': width > 500 }" class="flex flex-col basis-full mx-1">
      <Transition appear>
        <span
            v-if="['correct', 'incorrect', 'final-incorrect'].includes(state.state) && options.maxAttempts !== 0"
            class="font-medium"
            :class="titleColor"
        >
          {{ title }}
      ...
      <Transition appear>
        <div v-if="['incorrect'].includes(state.state) && options.maxAttempts !== 0">
          <div
              v-for="hint in hints"
              :key="hint.id"
              :class="fontColor"
    ...
    <div class="mt-2 flex flex-row gap-1 pr-2 items-center">
      ...
      <div
        ...
        >
          ...
          01-6.085-.711l-.108-.054a9 9 0 00-6.208-.682L3 4.5M3 15V4.5" stroke-linecap="round"
                  stroke-linejoin="round"/>
          </svg>
        </button>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const backgroundColor = computed(() => {
      return {
        "bg-green-50": props.state.state === "correct" && props.options.maxAttempts !== 0,
        "bg-yellow-50": props.state.state === "incorrect" && props.options.maxAttempts !== 0,
        "bg-red-50": props.state.state === "final-incorrect" && props.options.maxAttempts !== 0
      };
    });
